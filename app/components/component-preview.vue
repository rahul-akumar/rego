<template>
  <div class="rounded-xl border border-white/10 bg-neutral-900/40 shadow-sm overflow-hidden">
    <!-- Header (optional) -->
    <div v-if="title || description" class="px-4 py-3 border-b border-white/10">
      <p v-if="title" class="text-sm font-medium text-white">{{ title }}</p>
      <p v-if="description" class="text-xs text-white/60">{{ description }}</p>
    </div>
    ...
    <div v-if="hasCode" class="px-4 pb-4 border-t border-white/10">
      <div class="flex items-center justify-between">
        <h4 class="text-xs font-medium uppercase tracking-wide text-white/70">Usage</h4>
        <UiButton
          v-if="showCopyButton"
          size="xs"
          variant="outline"        
      ...
      <div class="mt-2">
        <template v-if="code">
          <pre class="p-3 overflow-x-auto text-[12px] leading-relaxed text-white/80"><code :class="language ? `language-${language}` : undefined">{{ code }}</code></pre>
        </template>
        <template v-else>
          <div class="p-3">
            <slot name="code" />
    ...
    <!-- Footer (optional) -->
    <div v-if="$slots.footer" class="px-4 py-3 border-t border-white/10 bg-neutral-900/40">
      <slot name="footer" />
    </div>
  </div>
</template>
