<template>
  <component :is="as" :class="[
  ...
  ]" :type="isButton ? type : undefined" :href="as === 'a' ? href : undefined" :aria-busy="loading || undefined"
    :aria-disabled="isDisabled || undefined" :disabled="isButton ? isDisabled : undefined" v-bind="$attrs"
    @click="onClick">
    ...
    <span class="ds-btn__inner" :class="{ 'is-invisible': loading }">
      <span v-if="$slots.start" class="ds-btn__icon ds-btn__icon--start" aria-hidden="true">
        <slot name="start" />
      </span>
      <span v-if="hasText" class="ds-btn__content">
        <slot />
      </span>
      <span v-if="$slots.end" class="ds-btn__icon ds-btn__icon--end" aria-hidden="true">
        <slot name="end" />
      </span>
    </span>
  </component>
...
.ds-btn--icon-only {
  --_size: calc((var(--_btn-font-size) * var(--_btn-line-height)) + (var(--_btn-padding-y) * 2) + 2px
      /* border */
    );
  width: var(--_size);
  height: var(--_size);
...
.ds-btn__icon {
  ...
  height: 1em;
  font-size: 1.2em;
  /* Slightly larger than text for visual balance */
}
